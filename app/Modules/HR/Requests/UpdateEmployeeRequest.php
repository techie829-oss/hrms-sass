...
class UpdateEmployeeRequest extends FormRequest
{
    ...
    {
        $employeeId = $this->route('employee'); // assuming the route parameter is named 'employee'

        return [
            'employee_id' => ['required', 'string', 'max:20', Rule::unique('employees')->where('tenant_id', saas_tenant('id'))->ignore($employeeId)],
            'first_name' => ['required', 'string', 'max:255'],
            'last_name' => ['required', 'string', 'max:255'],
            'email' => ['required', 'email', Rule::unique('employees')->where('tenant_id', saas_tenant('id'))->ignore($employeeId)],
