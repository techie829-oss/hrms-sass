...
@section('title', 'Edit Custom Role - ' . $role->name)

@section('content')
<div class="w-full space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Edit Role: <span class="capitalize text-primary-600">{{ $role->name }}</span></h1>
            <p class="text-xs text-gray-500 mt-1">Modify role name and updated permission checkboxes.</p>
        </div>
    ...
    <form action="{{ route('tenant.roles.update', $role->id) }}" method="POST" class="bg-white border border-gray-200 rounded-2xl p-6 shadow-xs space-y-6">
        @csrf
        @method('PUT')

        <div>
            <label for="name" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">Role Name</label>
            <input type="text" name="name" id="name" required value="{{ old('name', $role->name) }}" class="w-full text-xs rounded-lg border-gray-300 shadow-xs focus:border-primary-500 focus:ring-primary-500">
            @error('name')
        ...
        <div>
            <h3 class="text-xs font-bold text-gray-700 uppercase tracking-wider mb-4">Module Permissions</h3>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                            @foreach($groupPermissions as $permission)
                                <label class="flex items-center space-x-2 text-xs text-gray-700 cursor-pointer bg-white p-2 rounded-lg border border-gray-200 hover:border-primary-300">
                                    <input type="checkbox" name="permissions[]" value="{{ $permission->name }}" {{ in_array($permission->name, $rolePermissions) ? 'checked' : '' }} class="rounded border-gray-300 text-primary-600 focus:ring-primary-500">
