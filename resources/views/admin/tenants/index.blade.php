<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.tenants.create') }}" class="btn btn-primary bg-gradient-to-br from-primary to-tertiary border-none rounded-xl font-bold text-xs uppercase tracking-widest px-6 shadow-lg">
                    ...
                    <td class="py-6 px-8">
                        <div class="flex items-center gap-4">
                            <div class="avatar placeholder">
                                <div class="bg-primary/10 text-primary rounded-2xl w-12 h-12 font-black text-sm shadow-xl border border-primary/10 flex items-center justify-center">
                                    {{ strtoupper(substr($tenant->name, 0, 2)) }}
                                </div>
                            </div>
                            <div>
                                <div class="font-black text-on-surface text-base group-hover:text-primary transition-colors">{{ $tenant->name }}</div>
                                <div class="text-xs text-on-surface-variant font-medium opacity-70">{{ $tenant->email }}</div>
                                <div class="text-[10px] text-primary/70 font-bold uppercase tracking-widest mt-0.5">{{ $tenant->contact_no ?: 'No Contact' }}</div>
                            </div>
                        </div>
                    </td>
                    <td class="py-6 px-8">
                        <div class="font-mono text-xs font-bold text-primary bg-primary/5 px-3 py-2 rounded-lg border border-primary/10 inline-block">{{ $tenant->slug }}.{{ parse_url(config('app.url'), PHP_URL_HOST) ?? config('app.url') }}</div>
                    </td>
                    <td class="py-6 px-8">
                        <span class="badge {{ $tenant->mode === 'dedicated' ? 'badge-secondary' : 'badge-ghost' }} font-black text-[9px] uppercase tracking-widest px-3 py-3 h-auto border-none shadow-sm">
                            {{ $tenant->mode }}
                        </span>
                    </td>
                    <td class="py-6 px-8">
                        <span class="badge badge-outline text-primary border-primary/30 font-black text-[9px] uppercase tracking-widest px-3 py-3 h-auto">
                            {{ $tenant->plan_id }}
                        </span>
                    </td>
                    <td class="py-6 px-8">
                        <span class="badge {{ $tenant->status === 'active' ? 'badge-success' : 'badge-error' }} text-white font-black text-[9px] uppercase tracking-widest px-3 py-3 h-auto border-none shadow-sm shadow-black/5">
                            {{ $tenant->status }}
                        </span>
                    </td>
