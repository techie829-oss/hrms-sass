<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-3xl font-black font-headline tracking-tight text-on-surface uppercase">Edit Role: {{ str_replace('_', ' ', $role->name) }}</h2>
                <p class="text-sm text-on-surface-variant font-medium mt-1">Modify permissions and configuration for this access level.</p>
            </div>
        </div>
    </x-slot>
        ...
        <form action="{{ route('admin.roles.update', $role->id) }}" method="POST" class="space-y-8">
            ...
            <div class="bg-surface-container-lowest p-8 md:p-12 rounded-[3rem] shadow-xl border border-outline-variant/15 space-y-8">
                <div class="space-y-2">
                    ...
                    <x-text-input id="name" name="name" type="text" class="block w-full" :value="old('name', $role->name)" required oninput="this.value = this.value.replace(/\s+/g, '_').toLowerCase()" />
