        ...
        <form action="{{ route('admin.roles.store') }}" method="POST" class="space-y-8">
            ...
            <div class="bg-surface-container-lowest p-8 md:p-12 rounded-[3rem] shadow-xl border border-outline-variant/15 space-y-8">
                <div class="space-y-2">
                    ...
                    <x-text-input id="name" name="name" type="text" class="block w-full" required placeholder="system_auditor" oninput="this.value = this.value.replace(/\s+/g, '_').toLowerCase()" />
                                ...
                                <div class="space-y-3">
                                    @foreach($perms as $permission)
                                        <label class="flex items-center gap-3 cursor-pointer group">
                                            ...
                                            <span class="text-sm font-medium text-on-surface group-hover:text-primary transition-colors capitalize">{{ str_replace('-', ' ', $permission->name) }}</span>
                                        </label>
                                    @endforeach
                                </div>
                            </div>
